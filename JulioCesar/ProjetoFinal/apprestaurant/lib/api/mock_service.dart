...
class MockService {
  ...
  Future<List<Restaurant>> _getRestaurants() async {
    final response = await http.get(Uri.parse('https://68f68cfa6b852b1d6f171151.mockapi.io/app-restaurant/restaurants'));
  ...
  Future<List<FoodCategory>> _getCategories() async {
    final response = await http.get(Uri.parse('https://68f68cfa6b852b1d6f171151.mockapi.io/app-restaurant/categories'));
    if (response.statusCode == 200) {
      final List<dynamic> jsonList = jsonDecode(response.body);
